...
1. **Ingest.** Entry: `api.WireIngest` builds the pipeline, `Ingester.Handle`
   ...
   (server-side re-scrub of the message and every attribute value) ->
   ingest/normalize (classify canonical event names; keep `level_raw` as the
   ...
   scanner group on) -> ingest/cardinality (cap unseen distinct values) ->
