...
lease); key_usage_log, plan_entitlement, tenant_line_ledger (no writer in
the core; the ledger is read by ucworker's cutoff recompute, the write
query exists but is uncalled); billing_subscription (no writer in the
core).
...
**The seq ring is unchanged.** `project_window.cutoff_seq`/`retain_seq`
still define the per-plan visible window, recomputed from the line ledger,
and every read still filters `seq >= cutoff_seq`. Day partitions are only
the physical floor, not the retention model.
