...
| ucapi | The public door: the `/v1/*` API, public and hook surfaces, the schemaless ingest endpoint `POST /i`, and the ProbeService the probe fleet calls. It owns the whole ingest pipeline into ClickHouse and all auth doors. It enqueues deliveries, and with `--with-worker` (`UC_WITH_WORKER`, the default on a self-hosted install) it also runs the background jobs in-process: every job takes an advisory lock, so replicas stay safe either way. |
| ucworker | The background driver: the delivery queue (Telegram, Discord, Slack, email), retention recompute, expired-batch purging, the notification scanner, the log detector orchestrator and the heartbeat window job. Every job runs under a Postgres advisory lock, so N replicas never duplicate work. |
...
normalize, seq, cardinality, batcher into ClickHouse), all auth doors
...
(a missed ping is a failed check). The error-log and detector jobs
...
| internal/worker | the background jobs, every one under an advisory lock: delivery queue, retention recompute, purge, reaper, error-log scan, heartbeat, detection. Called by ucworker always, and by ucapi with --with-worker | x | x | |
...
1. **Ingest.** Entry: `api.WireIngest` builds the pipeline, `Ingester.Handle`
   ...
   size, age and a one-insert-per-second-per-key floor) -> storage/ch (batch
   INSERT into logs, plus a copy of classified event rows into events). The
...
- 001_init: alert_channel, api_key, ingest_batch, monitor, monitor_facts,
  monitor_schedule, person, project, project_seq, session, source_connection,
  status_page, tenant, tenant_member, webhook_seen (ucapi);
  delivery_attempt, project_window (ucworker); delivery_queue, incident,
  incident_slice, incident_update (ucapi+ucworker);
...
- 027_remove_ai: creates nothing; it drops ai_call, ai_explain_cache and
  ai_usage, which is why the entries above no longer list them.
- Migrations 002, 004-010, 012-014, 018, 020, 022, 024-026 alter existing
  tables or backfill data and create nothing.
- 028_heartbeat_ping_token: data-only; backfills `monitor.ping_token` (ucapi
  mints it on create) and opens a first window for pre-existing heartbeat
  monitors. `monitor_schedule.next_due_at` doubles as the heartbeat's
  "missed after" deadline.

**ClickHouse**

- 001_init: logs (ucapi via the batcher; `level_raw` keeps the level exactly
  as the client sent it, capped at 32 bytes, and `fingerprint` is computed at
  ingest as FNV-64a of the masked message), events (ucapi: the batcher's event
...
- 005_level_raw: adds `logs.level_raw`.
...
- The ring/query QueryBuilder methods CutoffSeq, Slice and BeyondErrors: no
  caller outside the package, but each carries a unit test that dies with it.
